...
import UIKit
  ...
  class CacheCleanerPlugin: NSObject,FBMemoryProfilerPluggable {
    func memoryProfilerDidMarkNewGeneration() {
      NSLog("memoryProfilerDidEnable")
    }
  }
  class RetainCycleLoggerPlugin: NSObject,FBMemoryProfilerPluggable {
    func memoryProfilerDidFindRetainCycles(retainCycles: Set<NSObject>) {
...
class AppDelegate: UIResponder, UIApplicationDelegate {
  ...
  func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
    // Override point for customization after application launch.
      ...
      memoryProfiler = FBMemoryProfiler(plugins: [CacheCleanerPlugin(),
        RetainCycleLoggerPlugin()], retainCycleDetectorConfiguration: nil)
